    ...
    fn instructor(&mut self, instruction: u8) {
        match instruction {
            ...
            I_EXP => {
                ...
                if let (UInteger(v2), Integer(v1)) = (handle_stack_err(self.stack.pop()), handle_stack_err(self.stack.pop())) {
            ...
            F_EXP => {
                ...
                if let (Float(v2), Float(v1)) = (handle_stack_err(self.stack.pop()), handle_stack_err(self.stack.pop())) {
            ...
            F_I_EXP => {
                ...
                if let (Integer(v2), Float(v1)) = (handle_stack_err(self.stack.pop()), handle_stack_err(self.stack.pop())) {
            ...
            0x19 => {
                ...
                async fn create_new_thread(heap: Arc<RwLock<VMHeap>>, repo: Arc<RwLock<VMRepository>>, threadnum: usize) -> Result<(), String>{

                    let mut thread = VMThread {
                        running: true,
                        pc: 0,
                        instruction: 0,
                        instructions: vec![0x0A, 0x0A, 0x01, 0x14, 0x0A, 0x0A, 0x01, 0x14, 0x0A, 0x0A, 0x01, 0x14, 0x0A, 0x0A, 0x01, 0x14, 0x01,  0],
                        data: Null,
                        data_vault: vec![Integer(1), Integer(1), Null, Null, Integer(1), Integer(1), Null, Null, Integer(1), Integer(1), Null, Null, Integer(1), Integer(1), Null, Null, Null, Null],
                        stack: VMStack::new(),
                        heap: &heap,
                        repository: &repo,
                    };
